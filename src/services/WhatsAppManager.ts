...
export class WhatsAppManager {
  ...
  async createConnection(userId: string, phoneNumber?: string, usePairingCode: boolean = true): Promise<UserSession> {
    ...
    try {
      ...
      // For new sessions (not registered), ensure QR generation is triggered
      if (!session.state.creds.registered) {
        userLogger.info('New session detected - QR code will be generated automatically')
        
        // Set a timeout to check QR generation status
        setTimeout(() => {
          if (!session.qrCode && !session.isConnected) {
            userLogger.warn('QR code not generated yet - this may indicate a connection issue')
          } else if (session.qrCode) {
            userLogger.info('QR code successfully generated for new session')
          }
  ...
  async forceQRGeneration(userId: string): Promise<boolean> {
    try {
      const session = this.sessionManager.getSession(userId)
      const socket = this.activeSockets.get(userId)
      
      if (!session || !socket) {
        logger.warn(`Cannot force QR generation - session or socket not found for user: ${userId}`)
        return false
      }

      if (session.isConnected) {
        logger.info(`User ${userId} is already connected - no QR needed`)
        return false
      }

      if (session.state.creds.registered) {
        logger.info(`User ${userId} is already registered - no QR needed`)
        return false
      }

      // If QR is already available, return true
      if (session.qrCode) {
        logger.info(`QR code already available for user: ${userId}`)
        return true
      }

      logger.info(`Attempting to force QR generation for user: ${userId}`)
      
      // The QR code should be generated automatically by Baileys when the connection is established
      // We can't manually generate it, but we can check if the connection is properly set up
      return false // QR will be generated by the connection.update event
    } catch (error) {
      logger.error({ error, userId }, `Failed to force QR generation for user ${userId}`)
      return false
    }
  }
}